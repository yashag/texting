...
function comments(comments) {
    return texting(comments, {
        lineStart: SLASH.repeat(2) + SPACE
    });
}
...
function multiComments(comments) {
    return texting(comments, {
        blockStart: SLASH + ASTERISK,
        blockEnd: ASTERISK + SLASH
    });
}
...
function jsdocComments(comments) {
    return texting(comments, {
        blockStart: SLASH + ASTERISK.repeat(2),
        blockEnd: SPACE + ASTERISK + SLASH,
        lineStart: SPACE + ASTERISK + SPACE
    });
}
...
function list(listItems) {
    return texting(listItems, {
        lineStart: DASH + SPACE
    });
}
